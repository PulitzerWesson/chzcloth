...
const INACTION_OPTIONS = [
  { value: 'lose-competitive-edge', label: 'Lose competitive edge' },
  { value: 'lose-opportunity',      label: 'Miss opportunity' },
  { value: 'risk-increases',        label: 'Risk increases' },
  { value: 'minor',                 label: 'Nothing bad happens' },
];
...
export default function BetSubmission({ onComplete, currentOrg }) {
  ...
  const [mustShipBy, setMustShipBy]           = useState('');
  ...
  // Auto-set mustShipBy from goal if available
  ...
  const handleReview = async () => {
    ...
    try {
      ...
      // Pre-fill effort from AI suggestion
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 0 60px 0' }}>
      ...
      {/* ── Goal Selection ─────────────────────────────────────────────────── */}
      {!hasLeadershipGoal && (
        <div style={{ marginBottom: 28, padding: '20px 24px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12 }}>
          ...
          {companyGoals.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ padding: '3px 8px', background: `${pColors[goal.priority]}22`, border: `1px solid ${pColors[goal.priority]}55`, borderRadius: 4, color: pColors[goal.priority], fontSize: '0.72rem', fontWeight: 700, flexShrink: 0 }}>
                          P{goal.priority}
                        </span>
                        <span style={{ color: isSelected ? '#f1f5f9' : '#cbd5e1', fontSize: '0.9rem', fontWeight: 500 }}>
                          {goal.title}
                        </span>
                        {goal.category && (
                          <span style={{ color: '#475569', fontSize: '0.78rem', marginLeft: 'auto', flexShrink: 0 }}>{goal.category}</span>
                        )}
                      </div>
                    </button>
                  );
                ...
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {selectedGoalKPIs.map((kpi, kpiIdx) => {
                      ...
                      return (
                        <label key={kpiIdx} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: isKpiSelected ? 'rgba(45,212,191,0.08)' : 'rgba(255,255,255,0.02)', border: `1px solid ${isKpiSelected ? 'rgba(45,212,191,0.3)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 8, cursor: 'pointer' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ color: '#f1f5f9', fontSize: '0.9rem', fontWeight: 500 }}>{kpi.metric}</div>
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: selectedKPI?.index === 'indirect' ? 'rgba(255,255,255,0.04)' : 'transparent', border: `1px solid ${selectedKPI?.index === 'indirect' ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.04)'}`, borderRadius: 8, cursor: 'pointer' }}>
            ...
            <>
              <input type="text" value={goalContext} onChange={e => setGoalContext(e.target.value)} placeholder="e.g., Grow revenue by 30% this year, Reduce churn to below 5%" style={inputStyle} />
      ...
      {hasLeadershipGoal && (
        <div style={{ marginBottom: 28, padding: '14px 18px', background: 'rgba(45,212,191,0.05)', border: '1px solid rgba(45,212,191,0.15)', borderRadius: 10 }}>
          <div style={{ color: '#475569', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>Supporting Leadership Goal</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24, alignItems: 'start' }}>
        ...
        <div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {!uploadedFile ? (
              <div style={{ position: 'relative' }}>
                ...
                <div style={{ padding: '20px', background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: 10, textAlign: 'center', cursor: 'pointer' }}>
                  <div style={{ fontSize: '1.4rem', marginBottom: 6 }}>📄</div>
                  <div style={{ color: '#64748b', fontSize: '0.85rem' }}>Click to upload or drag and drop</div>
                  <div style={{ color: '#334155', fontSize: '0.78rem', marginTop: 4 }}>PDF, Word, Text, or Markdown (max 32MB)</div>
                </div>
              </div>
            ) : (
          ...
          <div ref={reviewRef}>
            {submitState !== 'idle' && aiReview && (
              <div style={{ marginBottom: 20, padding: '20px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12 }}>
                ...
                {/* Weak issues warning when ready to score */}
          ...
          {/* Validation */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, alignItems: 'end', marginBottom: 8 }}>
            <div>
        ...
        <div style={{ position: 'sticky', top: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {/* Inaction Impact */}
          <div style={{ padding: '18px 20px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12 }}>
            <SectionLabel>If We Don't Do This</SectionLabel>
            <select value={inactionImpact} onChange={e => setInactionImpact(e.target.value)} style={selectStyle}>
              {INACTION_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </div>

          {/* Deadlines */}
          <div style={{ padding: '18px 20px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12 }}>
            <SectionLabel optional>Deadlines</SectionLabel>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div>
                <div style={{ color: '#475569', fontSize: '0.78rem', marginBottom: 6 }}>Start by</div>
                <input type="date" value={startBy} onChange={e => setStartBy(e.target.value)} style={{ ...inputStyle, colorScheme: 'dark' }} />
              </div>
              <div>
                <div style={{ color: '#475569', fontSize: '0.78rem', marginBottom: 6 }}>Must ship by</div>
                <input type="date" value={mustShipBy} onChange={e => setMustShipBy(e.target.value)} style={{ ...inputStyle, colorScheme: 'dark' }} />
              </div>
            </div>
          </div>
